...
public class LogoutHandlerMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext context, IServiceProvider serviceProvider)
    {
        if (context.Request.Path.Equals("/v1/auth/logout", StringComparison.OrdinalIgnoreCase))
        {
            ...
            var signInManager = serviceProvider.GetRequiredService<SignInManager<PoliceOfficer>>();
    ...
    private static async Task LogoutAsync(HttpContext context, ITokenRepository tokenRepository,
