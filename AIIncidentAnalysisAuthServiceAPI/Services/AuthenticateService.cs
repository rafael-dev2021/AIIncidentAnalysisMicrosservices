    ...
    ITokenManagerService tokenManagerService) : IAuthenticateService
{
    ...
    public async Task<IEnumerable<UserDtoResponse>> ListAllUsersServiceAsync()
    {
        Log.Information("[USERS] Fetching all users DTOs.");
        var usersDto = await repository.ListPoliceOfficersAsync();

        var enumerable = usersDto as PoliceOfficer[] ?? usersDto.ToArray();
    ...
    public async Task<TokenDtoResponse> UpdateUserServiceAsync(UpdateUserDtoRequest request, string userId)
    {
        ...
        var updateResponse = await repository.UpdateAsync(request, userId);
    ...
    public async Task<bool> ForgotPasswordServiceAsync(ForgotPasswordDtoRequest forgotPasswordDtoRequest)
    {
        Log.Information("[FORGOT_PASSWORD] Attempting to process forgot password for email: [{Email}]",
            forgotPasswordDtoRequest.Email);

        var result = await repository.ForgotPasswordAsync(forgotPasswordDtoRequest);
    ...
    private async Task<PoliceOfficer> GetUserByEmailAsync(string email)
    {
