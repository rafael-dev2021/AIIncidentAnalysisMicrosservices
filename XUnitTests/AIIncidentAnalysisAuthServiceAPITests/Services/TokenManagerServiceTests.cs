...
public class TokenManagerServiceTests
{
    ...
    private readonly Mock<IAuthRepository> _authenticatedRepositoryMock;
    ...
    public TokenManagerServiceTests()
    {
        ...
        _authenticatedRepositoryMock = new Mock<IAuthRepository>();
    ...
    {
        // Arrange
        var user = new PoliceOfficer { Id = "1", Email = "user@example.com" };
        user.SetName("Test");
        ...
        _tokenServiceMock.Setup(ts => ts.GenerateAccessTokenAsync(user)).Returns(Task.FromResult(accessToken));
        _tokenServiceMock.Setup(ts => ts.GenerateRefreshTokenAsync(user)).Returns(Task.FromResult(refreshToken));

        _tokenRepositoryMock.Setup(tr => tr.FindAllTokensByUserId(user.Id)).ReturnsAsync([]);
        _tokenRepositoryMock.Setup(tr => tr.SaveTokenAsync(It.IsAny<Token>())).ReturnsAsync((Token t) => t);
        _authenticatedRepositoryMock.Setup(ar => ar.SaveUserAsync(user)).Returns(Task.CompletedTask);
        ...
        _tokenServiceMock.Verify(ts => ts.GenerateAccessTokenAsync(user), Times.Once);
        _tokenServiceMock.Verify(ts => ts.GenerateRefreshTokenAsync(user), Times.Once);
        _authenticatedRepositoryMock.Verify(ar => ar.SaveUserAsync(user), Times.Once);
    }
    ...
    {
        // Arrange
        var user = new PoliceOfficer { Id = "1" };
