...
namespace XUnitTests.AIIncidentAnalysisAuthServiceAPITests.Endpoints.Strategies;
...
public class AuthenticationRulesTests
{
    ...
    public void CheckAdminRole_ReturnsNull_WhenUserIsAdmin()
    {
        ...
        var claims = new List<Claim> { new(ClaimTypes.Role, "Admin"), new(ClaimTypes.Role, "ReadWrite") };
        var identity = new ClaimsIdentity(claims, "TestAuthType");
        var principal = new ClaimsPrincipal(identity);
        context.User = principal;

        // Act
        var result = AuthenticationRules.CheckReadWriteRole(context);
